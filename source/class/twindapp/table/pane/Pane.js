 ...
 * scrolling.
 */
qx.Class.define("twindapp.table.pane.Pane", {
    ...
    */
    ...
    members: {
      ...
      _updateRowStyles(onlyRow) {
        ...
        //SQ var rowNodes = elem.firstChild.childNodes;
        var rowNodes = elem.firstChild;
        rowNodes = rowNodes.lastChild.childNodes;
        ...
        if (rowCount > 0) {
          ...
          if (table.getExcludeBoundsFromDom()) {
            ...
            for (var x = 0; x < colCount; x++) {
              ...
              //get next to last
              var nexttolast = colCount -2;
              var colname = tableModel.getColumnName(col);
              if (x == nexttolast) {
                var lastcolindx = x +1;
                var lastcol = paneModel.getColumnAtX(lastcolindx);
                if (tableModel.getColumnName(lastcol) == "NoShowLast"){
                  semHeader += `<th colspan="2" class='pb-4 pt-6 px-6'>${colname}</th>`;
                  break;
                } else {
                  semHeader += `<th class='pb-4 pt-6 px-6'>${colname}</th>`;
                }
              } else {
                semHeader += `<th class='pb-4 pt-6 px-6'>${colname}</th>`;
              }
            ...
            htmlArr = [
              ...
              "<thead><tr class='text-left font-bold'>",
              semHeader,
              "</tr></thead>",
              "<tbody>",
      ...
      getRenderedHeaderRowHeight() {
        ...
        if (elem && elem.firstChild) {
          // pane has been rendered
          var tableBody = elem.firstChild;
          //SQ new - get theader tag
          tableBody = tableBody.firstChild;
